 ...
 */
function getYearWeekNumber(d: Date) {
...
function groupByMonthWeek(arr: Bookings["bookings"]) {
  const grouped: YearMonthWeekBookings = {};
  arr.forEach((o) => {
    const date = new Date(o.date);
    const year = date.getUTCFullYear();
    const month = date.getMonth();
    const week = getYearWeekNumber(date)[1];
    if (!grouped[year]) {
      grouped[year] = {};
    }
